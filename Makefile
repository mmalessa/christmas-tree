...
go-modtidy: # Run go mod tidy
	@echo 'go mod tidy...'
	@docker run --rm -v "$(PWD)":/usr/src/$(APP_NAME) --platform linux/arm/v7 \
  	-w /usr/src/$(APP_NAME) ws2811-builder:latest go mod tidy

go-build: ## Run go build
	@echo 'go build -o "bin/$(APP_NAME)"...'
	@docker run --rm -v "$(PWD)":/usr/src/$(APP_NAME) --platform linux/arm/v7 \
  	-w /usr/src/$(APP_NAME) ws2811-builder:latest env CGO_ENABLED=1 go build -o "bin/$(APP_NAME)" -v
...
rpi-install: ## Send binary to RPI
	@echo "Send binary to RPI"
